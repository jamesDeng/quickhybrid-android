...
import com.quick.jsbridge.util.io.IOUtil;
import com.quick.jsbridge.bridge.Callback;
import com.quick.jsbridge.view.IFileChooseActivity;
...
public class PageLoad implements ILoadPage {
    ...
    private int statusPageError;

    private int statusTimeoutError;

    private IFileChooser fileChooser;

    public PageLoad(IQuickFragment fragment, IFileChooseActivity fileChooseActivity, int statusPageError, int statusTimeoutError) {
        this.fragment = fragment;
        this.statusPageError = statusPageError;
        this.statusTimeoutError = statusTimeoutError;
        fileChooser = new FileChooser(fragment,fileChooseActivity);
    }
    ...
    public void forwardUrl(final WebView view, final String url) {
        ...
        if (fragment.getPageControl().getNbBar() != null) {
            ...
            if (!viewHolder.nbLeftIv2.isShown() && !viewHolder.nbLeftTv1.isShown()) {
                ...
                viewHolder.nbLeftIv2.setImageResource( BaseThemeControl.getInstance().getSelectedTheme().navExitBtnImg);
            }
        }
    }
...
//        view.loadUrl(WebloaderControl.BLANK);
        if (url.equals(fragment.getQuickBean().pageUrl) || url.equals(fragment.getQuickBean().pageUrl + "/")) {
            //如果当前页面报错则
            fragment.getPageControl().getStatusPage().showStatus(statusPageError);
        }
        cancelTimeKeeper();
    }
            ...
            public void run() {
                ...
                view.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        fragment.getPageControl().getStatusPage().showStatus(statusTimeoutError);
                    }
                });
            }
        }, TIME_OUT, 1);
